...
public class TradeServiceAssistLogicImpl implements TradeServiceAssistLogic {
    ...
    /**
     * 1、事务传播级别是REQUIRED，当save()被调用时（假定被另一类中commit()调用）,
     *     如果save()中的代码抛出异常，即便被捕获，commit()中的其他代码都会roll back
     *  2、是REQUIRES_NEW，如果save()中的代码抛出异常，并且被捕获，commit()中的其他代码不会roll back；
     *            如果commit()中的其他代码抛出异常，而且没有捕获，不会导致save()回滚
     *  3、是NESTED，如果save()中的代码抛出异常，并且被捕获，commit()中的其他代码不会roll back；
     *         如果commit()中的其他代码抛出异常，而且没有捕获，会导致save()回滚
     */
    @Override
    @Transactional(propagation=Propagation.REQUIRES_NEW)
